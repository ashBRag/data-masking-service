...
async def lifespan(app: FastAPI):
    """Log startup/shutdown and verify infra connectivity (non-fatal if any of it fails)."""
    logger.info(
        "application_startup",
    ...
    # Connect to each dependency independently so one being down doesn't stop
    # the others from connecting - /health reports each one's live status,
    # which is more useful in a boilerplate where infra may come up on its
    # own schedule.
    try:
        await db.connect()
...
async def health_check(request: Request) -> dict[str, Any]:
    """Liveness + dependency connectivity check (Postgres, S3).
    ...
    s3_healthy = await s3.health_check()
    all_healthy = db_healthy and s3_healthy
